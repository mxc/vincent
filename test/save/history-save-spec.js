...
describe("HistoryManager should",function(){
    ...
    it("save data correctly.", function () {
        let entry1 = {
            plays: [
                {
                    play: {
    ...
    it("import data correctly.", function () {
        ...
        let history2 = new History(provider.engine,"192.168.122.21");
        let date1 = Date.parse("2016-08-21");
        let date2 = Date.parse("2016-09-14");
        history2.addEntry(date1, entry1);
        history2.addEntry(date2, entry2);
        historyManager.save(history2);
        var result = fs.statSync(`${provider.getDBDir()}/history/${history2.configGroup}/${history2.name}.json`);
